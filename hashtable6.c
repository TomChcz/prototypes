...
void unload(hashnode* hashtable[]);
void deleteList(hashnode* ptr);
...
int main(void)
{
    ...
    while(fgets(word, NAMELENGTH_MAX+1, infilePtr) != NULL)
    {
        ...
        // insert node to hashtable position
...
// unloads hashtable from memory

void unload(hashnode* hashtable[])
{
    // loop through the hashtable positions
...
// deletes linked list recursively
void deleteList(hashnode* ptr)
{
    // if head of list not empty
    if(ptr != NULL)
    {
        // if node points to next node
        if(ptr->next !=NULL)
        {
            // go delete the next node
            deleteList(ptr->next);
        }
